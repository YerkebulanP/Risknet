  ...
  <v-navigation-drawer
    ...
    <v-toolbar flat dark :color="$root.themeColor" class="toolbar">
      <router-link :to="{ name: 'Home' }">
        <img src="static/ktz_logo.png" width="250px">
      </router-link>
    ...
    <v-list>
      ...
      <v-list-group
        ...
        <v-list-tile slot="activator">
          <v-list-tile-title class="item-title">{{ $t('Реестр') }}</v-list-tile-title>
        </v-list-tile>
        
        <v-list-tile @click="changeRoute('Table', 4)">
          <v-list-tile-action>
            <v-icon>group</v-icon>
          </v-list-tile-action>
          <v-list-tile-title :class="[{'active': selectedIndex === 4}, 'item-title' ]">{{ $t('Таблица') }}</v-list-tile-title>
        </v-list-tile>
        <v-list-tile @click="changeRoute('Chart', 5)">
          <v-list-tile-action>
            <v-icon>bar_chart</v-icon>
          </v-list-tile-action>
          <v-list-tile-title :class="[{'active': selectedIndex === 5}, 'item-title' ]">{{ $t('Графики') }}</v-list-tile-title>
        </v-list-tile>
    </v-list-group>

    <v-list-tile @click="changeRoute('risk-appetite', 2)">
        <v-list-tile-action>
          <v-icon>calendar_today</v-icon>
        </v-list-tile-action>
        <v-list-tile-title :class="[{'active': selectedIndex === 2}, 'item-title' ]"> {{ $t('Риск-аппетит') }}</v-list-tile-title>
      </v-list-tile>
      <!-- <v-list-group
